...
interface AnxiousQuickActionsProps {
    accent: string;
}
    ...
    return (
        <div className="bento-section">
            <div className="bento-header" style={{ marginBottom: 24, display: 'flex', alignItems: 'center', gap: 16 }}>
                {mode && (
                    <button onClick={() => { setMode(null); setWorryCaptured(false); }} style={{ background: 'transparent', border: 'none', color: '#94a3b8', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                        <ArrowLeft />
                    </button>
                )}
                ...
                {mode === 'calm' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}>
                            {renderToolBtn("Calm in 60s", () => setMode('exit'))}
                            {renderToolBtn("Cyclic sigh breathing", () => setMode('exit'))}
                            {renderToolBtn("Sensory grounding", () => setMode('exit'))}
                            {renderToolBtn("PMR short scan", () => setMode('exit'))}
                        </div>
                    </div>
                )}
