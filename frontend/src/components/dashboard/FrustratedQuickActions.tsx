...
interface FrustratedQuickActionsProps {
    accent: string;
}
                ...
                {!mode && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                            ...
                            {renderCard("Help Me Think Clearly", "For when you are stuck in an irritation or grievance loop.", () => setMode('think'))}
                ...
                {mode === 'cool' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 12 }}>
                            {renderToolBtn("Breathing resets", () => setMode('exit'))}
                            {renderToolBtn("Shake it out", () => setMode('exit'))}
                            {renderToolBtn("Jaw / shoulder release", () => setMode('exit'))}
                            {renderToolBtn("Cold water reset", () => setMode('exit'))}
                            {renderToolBtn("1-minute walk prompt", () => setMode('exit'))}
                        </div>
                    </div>
                )}
                ...
                {mode === 'respond' && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 12 }}>
                            {renderToolBtn("Delay response timer", () => setMode('exit'))}
                            ...
                            {renderToolBtn("Come back in 10 minutes", () => setMode('exit'))}
                        </div>
                    </div>
                )}
