...
interface FrustratedRecoveryProps {
    accent: string;
}
...
export function FrustratedRecovery({ accent }: FrustratedRecoveryProps) {
    ...
    const renderCard = (title: string, subtitle: string, content: React.ReactNode) => (
        <div className="frustrated-card" style={{ 
            ...
            <div className="frustrated-content flex flex-col gap-4">
                {content}
            </div>
        </div>
    );
    ...
    return (
        <div className="frustrated-recovery-wrapper" style={{ marginTop: 32, marginBottom: 32 }}>
            ...
            {step === 2 && branch === 'cool' && renderCard(
                "Cool me down first",
                "First, let the heat come down.",
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {renderButton("60-second breathing", () => handleNext(3))}
                    {renderButton("Shake it out", () => handleNext(3))}
                    {renderButton("Drop shoulders + unclench jaw", () => handleNext(3))}
                    {renderButton("Cold water / wash face", () => handleNext(3))}
                    {renderButton("1-minute walk", () => handleNext(3))}
                </div>
            )}

            {step === 2 && branch === 'think' && renderCard(
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                        <div style={{ border: `1px solid ${accent}40`, borderRadius: 16, padding: '20px 24px', background: 'rgba(2,6,23,0.4)' }}>
                            ...
                            <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 12, color: '#e2e8f0', fontSize: '0.95rem' }}>
                                <li>&mdash; My message</li>
                                <li>&mdash; My tone</li>
                                <li>&mdash; Whether I take a break</li>
                                <li>&mdash; My next step</li>
                            </ul>
                        </div>
                        <div style={{ border: `1px solid #475569`, borderRadius: 16, padding: '20px 24px', background: 'rgba(2,6,23,0.4)' }}>
                            ...
                            <ul style={{ listStyle: 'none', margin: 0, padding: 0, display: 'flex', flexDirection: 'column', gap: 12, color: '#94a3b8', fontSize: '0.95rem' }}>
                                <li>&mdash; What they already said</li>
                                <li>&mdash; Their personality</li>
                                <li>&mdash; What already happened</li>
                            </ul>
                        </div>
                    </div>
            ...
            {step === 5 && renderCard(
                "Choose the safest useful response",
                "What is the calmest useful next move?",
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                    {renderButton("Step away for 10 minutes", () => handleNext(6))}
            ...
            {step === 6 && renderCard(
                "Close the loop",
                "You do not need to solve this at full heat.",
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            {step === 7 && renderCard(
                "Session complete",
                "You successfully delayed an impulsive reaction.",
